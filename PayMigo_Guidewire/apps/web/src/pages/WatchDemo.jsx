...
import { Play, Pause, RotateCcw, Shield, Zap, Wallet, ArrowRight, CheckCircle2, X, Loader2, Key } from 'lucide-react';
  ...
  const generateDemoVideo = async () => {
    ...
    try {
      ...
      let operation = await ai.models.generateVideos({
        ...
        prompt: 'A cinematic 3D animation of a delivery rider on a scooter in a rainy city at night. A digital overlay appears showing a rainfall sensor hitting a threshold, then a mobile phone screen flashes with a notification: "₹1,500 Payout Received". High quality, professional lighting, 4k.',
        config: {
          numberOfVideos: 1,
          resolution: '720p',
          aspectRatio: '16:9'
  ...
  const demoSteps = [
    {
      ...
      description: "Our AI engine monitors weather and AQI data in your specific delivery zone 24/7.",
      icon: <Zap className="w-6 h-6 text-accent" />,
      timestamp: "0:05"
    },
    {
      title: "Automatic Trigger",
      description: "When conditions hit your plan's threshold (e.g., 15mm rain), the policy activates instantly.",
      icon: <Shield className="w-6 h-6 text-accent" />,
      timestamp: "0:12"
    },
    {
      title: "Instant Payout",
      description: "No claims, no paperwork. Funds are pushed to your Paymigo wallet in under 90 seconds.",
                    ...
                    {!hasKey ? (
                      <div className="space-y-6 max-w-md">
                        ...
                        <p className="text-white/60 text-sm leading-relaxed">
                          ...
                          This ensures high-quality, real-time visualization.
                        </p>
                        <button 
                        ...
                        <p className="text-[10px] text-white/40 uppercase tracking-widest font-bold">
                          Requires Google Cloud Paid Project
                        </p>
                      </div>
                    ) : (
                  ...
                  >
                    <div className="space-y-8 max-w-lg">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-accent font-mono text-xs uppercase tracking-[0.2em] h-4">
                          {loadingMessage}
                        </p>
                      </div>
              ...
              {videoUrl && !isGenerating && (
                <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <div className="text-white font-mono text-xs tracking-widest">
                    {videoRef.current ? `0:${Math.floor(videoRef.current.currentTime).toString().padStart(2, '0')}` : '0:00'} / 0:07
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="glass-card p-6 border-success/20">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-success">Live Status</span>
                </div>
                <p className="text-sm font-bold">Monitoring Active</p>
              </div>
              <div className="glass-card p-6">
                <p className="text-[10px] font-black uppercase tracking-widest text-text-secondary mb-2">Current Zone</p>
                <p className="text-sm font-bold">Chennai South-4</p>
              </div>
              <div className="glass-card p-6">
                <p className="text-[10px] font-black uppercase tracking-widest text-text-secondary mb-2">Active Plan</p>
                <p className="text-sm font-bold">Pro (₹119/wk)</p>
              </div>
            </div>
          </div>
